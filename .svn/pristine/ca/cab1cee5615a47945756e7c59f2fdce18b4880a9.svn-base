...
public class BreakManager {
    ...
    public String toString() {
        return String.format("Break Manager breaking %s nextBreakIn %s, stopping %s breakStopIn %s", breaking, nextBreakIn, stopping, breakStopIn);
    }

    private void stopBotFunc() {
        if (settings.stoppingEnabled()
                && settings.stopAfter() > 0
                && bot.getRuntimeMillis() > (settings.stopAfter() * 60000L))
            this.stopping = true;
    }
