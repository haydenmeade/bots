...
//            defenseRange += itemStatsEquipment.getDrange();
    ...
    public @Nullable Loadout equip(Loadout current, boolean force, @Nullable Collection<LoadoutOverrider> loadoutOverrider) {
        Stopwatch timer = Stopwatch.createStarted();
        ...
        for (var a : this.loadout.entrySet()) {
            var toEquip = getOverrideOrNot(loadoutOverrider, a);
            if (Inventory.contains(toEquip) && !Equipment.contains(toEquip))
                if (!util.equip(toEquip)) {
    ...
    private Pattern getOverrideOrNot(@Nullable Collection<LoadoutOverrider> loadoutOverrider, Map.Entry<Equipment.Slot, Pattern> a) {
    ...
    public Prayer[] getBoostPrayers() {
        return util.getBoostPrayersA(style);
    }
    ...
    public Map<Pattern, Integer> getBank(@Nullable Collection<LoadoutOverrider> loadoutOverriders) {
    ...
    public boolean isEquippedOrInInventory(Collection<LoadoutOverrider> overrides) {
        var items = util.inventoryEquipmentSource();
        return Items.containsAllOf(items, getBank(overrides).keySet().toArray(Pattern[]::new));
    }
