...
public class FightState extends BaseState<SireState> {
    ...
    public FightState(sireBot bot, Loadouts loadouts, SireListener sireListener, PrayerFlicker prayerFlicker) {
        super(bot, SireState.FIGHTING);
        this.bot = bot;
        this.loadouts = loadouts;
        this.sireListener = sireListener;
        this.prayerFlicker = prayerFlicker;
        this.dispatcher = bot.getEventDispatcher();
    }
    ...
    public void executeLoop() {
        ...
        switch (fs) {
            case Phase0:
                ...
                util.boostIfNeeded(phase1.getStyle(), bot.settings().reboost());
                ...
                if (!Execution.delayUntil(() -> sireNpc.getId() != sireBot.SireId_Phase0,
                        600, 1200)) {
                    if (bot.settings().initiateWithStun())
                        stunSire();
                    else
                        sire.attack();
                }
    ...
    public @Nullable Coordinate attackClosestRespiratory() {
        ...
        if (nearestResp != null && sireBot.fightArea.contains(nearestResp) && nearestResp.isValid()) {
            ...
            if (trgt == null || !pos.equals(trgt.getPosition())) {
                ...
                di.send(MenuAction.forNpc(nearestResp, "Attack"));
            }
    ...
    public boolean stunSire() {
        ...
        di.sendSpellCastOn(Magic.Ancient.SHADOW_BARRAGE, s);
