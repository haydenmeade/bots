...
public class MinotaurOverride implements TraverseOverride {
    ...
    public boolean overrideLoop(NeckBot<?, ?> bot, Coordinate destination) {
        ...
        var monster = Monster.getByEnum(Task.MINOTAURS).get();
        var loc = monster.getLocation(bot);
        if (loc.getArea().contains(pos)) return true;
        var stronghold = 7505;
        if (pos.getContainingRegionId() != stronghold) {
            var entrance = GameObjects.newQuery()
                    ...
                    .first();
            if (entrance != null) {
                bot.di.send(MenuAction.forGameObject(entrance, "Climb-down"));
        ...
        if (nextDoor != null) {
            bot.di.send(MenuAction.forGameObject(nextDoor, "Open"));
