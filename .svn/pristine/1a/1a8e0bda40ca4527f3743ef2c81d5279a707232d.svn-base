    ...
    public SlayerState getNextState(SlayerState initiator) {
        ...
        switch (initiator) {
            ...
            case LOOTING:
                ...
                if (bot.doWeNeedToRestock() || bot.breakManager.breaking() || bot.breakManager.stopBot() || Loot.cantLootMore(bot.getCannon().isCannonPlaced() ? 4 : 0)) {
                    return SlayerState.RESTORING;
                } else {
        ...
        if (loc.getArea().contains(Players.getLocal())) {
            if (util.anyOtherPlayersWithin(loc.getArea(), util.parseCsvRegexString(bot.settings().ignorePlayers())))
