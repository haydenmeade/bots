...
public class FightState extends BaseState<HydraState> implements EngineListener {
    ...
    public void onTickStart() {
        try {
            ...
            if (hydra.getHp() < 50 && bot.settings().braceletsOfSlaughter() > 0 && !equipedSlaughter) {
                util.equip(items.braceletOfSlaughter);
    ...
    private void phaseStep(@NotNull Hydra hydra) {
        ...
        if (!currentPhase.equals(phase)) {
            var oldIdx = currStep == null ? 0 : currStep.getIndex();
            currentPhase = phase;
            ticksInStep = 0;
            ...
            log.debug(String.format("New Phase %s Step || oldIdx %s, newIdx %s, loc %s", hydra.getPhase(), oldIdx, currStep.getIndex(), currentMoveTo));
        }
    }
