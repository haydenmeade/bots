...
public class NeckInventory {
    ...
    private static boolean isBoost(ItemDefinition def) {
        return Arrays.stream(Boost.values()).anyMatch(b -> Arrays.stream(b.patternAny()).anyMatch(p -> p.matcher(def.getName()).find()));
    }
