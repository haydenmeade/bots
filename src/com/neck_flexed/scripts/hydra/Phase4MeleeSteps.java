...
public class Phase4MeleeSteps implements PhaseSteps {

    private final Coordinate step0;
    private final Coordinate step1;

    public Phase4MeleeSteps() {
    ...
    public @NotNull PhaseSteps.Step GetNextStep(int idx) {
        if (!Traversal.isRunEnabled())
            Traversal.toggleRun();
        if (idx % 2 == 0) {
            return new Step(0, h -> (h.getTick() + 2) == h.getSpecialAttackTick() || isOnPoison(), h -> {
            ...
            return new Step(1, 0,
                    h -> {
                        ...
                        for (int y = 0; y < bestSecond.getY(); y++) {
                            int finalY = y;
                            if (danger.stream().noneMatch(c ->
                                    c.equals(bestSecond.derive(0, finalY)) && c.equals(step0.derive(0, finalY)
                                    ))) {
