...
public class RestockState extends BaseState<SlayerState> {
    ...
    private int getBestRuneCount(Rune r, int min, int max) {
        var onMe = Inventory.getQuantity(r.getName()) + (Inventory.contains(items.runePouch) ? RunePouch.getQuantity(r) : 0);
    ...
    protected int getRuneCountTotal(Rune r) {
        return Inventory.getQuantity(r.getName()) + Bank.getQuantity(r.getName());
    }
