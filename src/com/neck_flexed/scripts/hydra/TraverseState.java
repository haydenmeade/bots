...
public class TraverseState extends BaseState<HydraState> {
    ...
    public TraverseState(com.neck_flexed.scripts.hydra.bot bot, Loadouts loadouts, PrayerFlicker prayerFlicker) {
        super(bot, HydraState.TRAVERSING);
        this.bot = bot;
        this.loadouts = loadouts;
        this.prayerFlicker = prayerFlicker;
        var h = new KaraluumDungeonOverrider();
        this.traverser = new Traverser(bot,
                bot.getHouseConfig(),
                destTile,
                Traverses.MOUNT_KARALUUM_REGIONS,
                destination -> h.overrideLoop(bot, destination),
                Traverses.MOUNT_KARALUUM_TRAVERSES
        );
    }
    ...
    public void executeLoop() {
        ...
        if (com.neck_flexed.scripts.hydra.bot.isInKaralumDungeon()) {
            ...
            if (r != null && pos.getY() >= 10228) {
                ...
                di.send(MenuAction.forGameObject(r, "Climb"));
                ...
                Execution.delay(600, 700);
                dungeonTraverser = null;
                di.send(MenuAction.forGameObject(s, "Enter"));
                Execution.delay(600, 700);
                Execution.delayUntil(() -> getRocks() != null && ChatDialog.getContinue() != null,
                        util::playerAnimatingOrMoving
                        , 20000, 25000);
            } else if (!canShortcut() && rFirst != null && pos.getY() < 10215) {
                log.debug("RocksFirst");
                dungeonTraverser = null;
                di.send(MenuAction.forGameObject(rFirst, "Climb"));
            ...
            log.debug("Elevator");
            di.send(MenuAction.forGameObject(de, "Activate"));
