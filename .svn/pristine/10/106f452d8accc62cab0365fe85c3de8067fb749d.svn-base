...
public class KqListener implements NpcListener, EngineListener {
    ...
    public void onTickStart() {
        var queen = Npcs.getLoaded("Kalphite Queen").first();
        if (queen == null) return;
        if ((this.phase == KqPhase.Transition && queen.getId() == kq.IdPhase2)
                && queen.getAnimationId() != phaseAnim && queen.getAnimationId() != hatchAnim) {
