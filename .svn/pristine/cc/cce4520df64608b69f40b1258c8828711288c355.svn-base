...
public class Phase2MeleeSteps implements PhaseSteps {
    ...
    public @NotNull PhaseSteps.Step GetNextStep(int idx) {
        switch (idx) {
            case 0:
                return new Step(0, h -> !h.getHydra().getImmunity() && h.playerAttacked(), HydraListener::isNotOverVent, step0);
            case 1:
                return new Step(1, HydraListener::playerAttacked, step1);
            case 2:
                return new Step(2, HydraListener::playerAttacked, step2);
            case 3:
                return new Step(3, h -> h.getHydra().getNpc().getServerPosition().getX() <= xx, h -> h.getHydra().getNpc().getServerPosition().getX() > xx, step3);
            case 4:
                return new Step(4, h -> {
                    if (h.getHydra().getNextSpecialRelative() < 3) return false;
                    var l = h.getHydra().getNearestLightning();
                    if (l == null) return false;
                    return l.distanceTo(Players.getLocal().getServerPosition()) <= 8.5;
                }, step4);
            case 5:
                return new Step(5, h -> {
                    if (h.getHydra().getNextSpecialRelative() < 3) return false;
            ...
            default:
                return new Step(2, h -> h.getHydra().getNextSpecialRelative() == 0, step4);
        }
    }
}
