...
public class LootState extends BaseState<SlayerState> implements ChatboxListener {
    ...
    public LootState(SlayerBotImpl<?> bot, LootSettings lootSettings, SlayerMonster monster, boolean buryBones) {
    ...
    public static @Nullable GroundItem getAvailableLoot(SlayerBot bot, LootSettings settings) {
        if (bot.getDeathTiles().isEmpty()) return null;
        bot.timeCheckDeathTiles();
        return Loot.getGroundItemToLoot(getLootArea(bot), settings);
    }

    public static boolean anyAvailableLoot(SlayerBot bot, LootSettings settings) {
        return getAvailableLoot(bot, settings) != null;
    }
    ...
    public void executeLoop() {
        ...
        if (this.buryBones) Loot.buryBonesScatterAshes(bot.di);
        ...
        if (sb != null && Inventory.getQuantity(ENSOULED_HEADS) > 0) {
    ...
    public void onMessageReceived(MessageEvent event) {
        if (event.getType().equals(Chatbox.Message.Type.SERVER) && Loot.IRONMAN_LOOT.matcher(event.getMessage()).matches()) {
            this.gotIronmanMessageOn = Players.getLocal().getServerPosition();
            log.warn("Got Ironman loot message on {} ({})", gotIronmanMessageOn, event.getMessage());
        }
    }
}
