...
public enum Food {
    ...
    public static boolean isFood(ItemDefinition s) {
        if (s == null || s.isNoted()) return false;
    ...
    public static boolean dropWorst() {
        ...
        if (minV != null)
            DI.get().send(MenuAction.forSpriteItem(minV, "Drop"));
        return true;
    }
    ...
    public static boolean dropAny() {
        var foods = Inventory
                ...
                .results();
        if (foods.isEmpty()) return false;
        DI.get().send(MenuAction.forSpriteItem(foods.first(), "Drop"));
        return true;
    }
