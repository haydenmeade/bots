...
import com.runemate.game.api.script.framework.listeners.events.ProjectileMovedEvent;
...
public class FightState extends BaseState<SlayerState> implements NpcListener, ProjectileListener, ChatboxListener {
    ...
    public static boolean doCannonLogic(Cannon cannon, SlayerBotImpl<?> bot, Coordinate cannonSpot, Coordinate playerPos, Area fightArea, Pattern[] ignorePlayers, boolean isHoppingDisabled) {
        if (!cannon.isCannonPlaced()) {
            if (util.anyOtherPlayersWithin(fightArea, ignorePlayers) && !isHoppingDisabled) {
                bot.updateStatus("Hopping when moving / setting up cannon.");
                bot.forceState(SlayerState.HOPPING);
                return true;
            }
            if (playerPos.equals(cannonSpot)) {
                cannon.place();
                ...
                util.moveTo(cannonSpot);
                Execution.delayUntil(
                        () -> Players.getLocal().getServerPosition().equals(cannonSpot),
                        util::playerMoving,
                        1000, 2000);
            }
            return true;
    ...
    public void onNpcDeath(DeathEvent event) {
        ...
        if (monster.getTask().equals(Task.CAVE_KRAKEN)) {
            bot.handleNpcDeath(monster, npc, currentTarget, Players.getLocal().getServerPosition());
        } else {
    ...
    public void executeLoop() {
        ...
        if (this.cannon != null) {
            if (doCannonLogic(cannon, bot, location.getCannonSpot(), pos, location.getArea(), ignorePlayers, monster.isHoppingDisabled()))
                return;
        ...
        if (shouldSpec()) {
            ...
            if (Objects.equals(bot.getPreviousState(), SlayerState.TRAVERSING)) {
                loadouts.invalidateCurrent();
            }
            loadouts.equip(loadout);
        }
    ...
    private void wakeUpMonster() {
        ...
        switch (t) {
            case CAVE_KRAKEN:
                var whirlpool = Npcs.newQuery()
                        .names("Whirlpool")
                        .actions("Disturb")
                        .results().nearest();
                if (whirlpool != null && whirlpool.isValid()) {
                    log.debug("Disturbing whirlpool {}", whirlpool);
                    di.send(MenuAction.forNpc(whirlpool, "Disturb"));
    ...
    private void checkLoot() {
        if (lootSettings != null && System.currentTimeMillis() - lastCheck > 2400) {
            ...
            if (l != null) {
                ...
                if (pos != null && this.monster.getTask().equals(Task.LIZARDMEN) && !bot.getShamanEncounter().isSafe(pos)) {
                    log.debug("Unsafe shaman state change, blocked");
                } else {
                    this.done = true;
                }
            }
    ...
    private void checkSinglesForTargetChange(Player p, Coordinate pos) {
        ...
        var attackingMe = Npcs.newQuery()
                ...
                .filter(n -> Objects.equals(p, n.getTarget()))
                .results().nearest();
        if (util.isMulti() && attackingMeOther != null) {
    ...
    @Override
    public void onProjectileMoved(ProjectileMovedEvent event) {
        ...
        if (Objects.equals(dodgeProjectile, projectile)) return;
        log.debug("We seen an Aoe Projectile: {} -- {} -- I'm on: {}", projectileInfo, projectile, p);
        this.dodgeProjectile = projectile;
        if (event.getDestination().equals(p))
            dodgeProjectile(projectileInfo, p);
    }

    private void dodgeProjectile(AoeProjectileInfo projectileInfo, Coordinate currentPosition) {
        var coordinates = (currentTarget != null)
                ? util.getMeleeCoordinates(currentTarget)
                : location.getArea().getCoordinates();

        var danger = currentPosition.getArea().grow(projectileInfo.getAoeSize(), projectileInfo.getAoeSize());
    ...
    private void dumpInfo(SlayerMonster monster, ProjectileMovedEvent event) {
        var projectile = event.getProjectile();
        if (projectile == null) return;
        if (currentTarget == null) return;

        log.debug("Projectile dump {} projectile {} target {} launch pos {}, anim {}, currentTarget {}, my position: {}, event destination {}",
                monster,
                projectile,
