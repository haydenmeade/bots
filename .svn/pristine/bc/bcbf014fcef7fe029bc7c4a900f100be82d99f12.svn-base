...
public class TraverseState extends BaseState<SarachnisState> {
    ...
    public TraverseState(com.neck_flexed.scripts.sarachnis.bot bot) {
        super(bot, SarachnisState.TRAVERSING);
        this.bot = bot;
    ...
    public void deactivate() {
        if (needToHop)
            bot.forceState(SarachnisState.HOPPING);
    }
    ...
    public void executeLoop() {
        ...
        if (inEntryArea() && bre != null) {
            ...
            if (bre.distanceTo(p) > 3) {
                bot.di.sendMovement(bre);
            ...
            if (p.getY() < 9933) {
                var web3 = getWeb3();
                if (web3 != null) {
                    di.send(MenuAction.forGameObject(web3, "Slash"));
                ...
                var web2 = getWeb2();
                if (web2 != null) {
                    di.send(MenuAction.forGameObject(web2, "Slash"));
                ...
                if (web1 == null && p.getY() > 9950) {
                    ...
                    di.send(MenuAction.forGameObject(web1, "Slash"));
            ...
            if (le == null) return;
            di.send(MenuAction.forGameObject(le, "Climb-down"));
