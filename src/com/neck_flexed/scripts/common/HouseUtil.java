...
public class HouseUtil {
    ...
    public static boolean JbToCastleWars() {
        ...
        if (!jb.getActiveDefinition().getActions().contains("Castle Wars")) {
            ...
            if (!DI.get().send(MenuAction.forGameObject(jb, "Castle Wars")))
    ...
    public static boolean restoreStatsInHouse(NeckBot bot) {
        ...
        try {
            if (!bot.di.send(MenuAction.forGameObject(pool, "Drink")))
                pool.interact("Drink");
            if (!Execution.delayUntil(l::hasRestored, util::playerAnimatingOrMoving, 2200, 2400)) {
