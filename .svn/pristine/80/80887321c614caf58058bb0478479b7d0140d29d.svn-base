...
public class util {
    ...
    public static boolean isMulti() {
        return VarbitID.MULTICOMBAT_AREA.getValue() == 1;
    }
    ...
    public static boolean anyOtherPlayersWithin(Area a, Pattern[] patterns) {
        var local = Players.getLocal();
        if (local == null) return false;
        var q = Players.newQuery();
        if (a != null) q = q.within(a);
        return !q.filter(p -> {
                    var isLocal = Objects.equals(p, local);
                    if (isLocal) return false;
                    var isIgnored = Arrays.stream(patterns).anyMatch(util.checkPatternArrayAgainst(p.getName()));
    ...
    public static boolean anyOtherPlayersWithin(Area a) {
        return anyOtherPlayersWithin(a, new Pattern[0]);
    }
    ...
    public static void attack(Npc npc) {
        ...
        DI.get().send(MenuAction.forNpc(npc, "Attack"));
    ...
    public static void activateSpec() {
        ...
        DI.get().send(MenuAction.forInterfaceComponent(component, "Use"));
    }

    public static boolean equipDramen() {
        var item = Inventory.getItems(items.dramenStaff).first();
        equip(item);
        return Execution.delayUntil(() -> Equipment.contains(items.dramenStaff), 1200);
    }

    public static boolean equip(SpriteItem item) {
        if (item == null) return false;
        return DI.get().send(MenuAction.forSpriteItem(item, Inventory.EQUIP_ACTIONS));
    }

    public static boolean equip(String p) {
        var item = Inventory.getItems(p).first();
        return equip(item);
    }

    public static boolean equip(Pattern p) {
        var item = Inventory.getItems(p).first();
        return equip(item);
    }
        ...
        if (!House.isInside()) {
            if (Inventory.contains(items.houseTab)) {
                ...
                if (item != null) {
                    ...
                    DI.get().send(MenuAction.forSpriteItem(item, "Inside"));
                }
                ...
                if (item != null) {
                    if (Equipment.contains(items.constructionCape)) {
                        ...
                        ControlPanelTab.INVENTORY.open();
                        DI.get().send(MenuAction.forSpriteItem(item, items.conCapeTelePoh));
                    }
                }
            } else {
    ...
    public static boolean isFullyRestored() {
        var isFerox = ferox.contains(Players.getLocal());
    ...
    public static void take(GroundItem gi) {
        ...
        DI.get().send(MenuAction.forGroundItem(gi, "Take"));
    }
    ...
    public static void ReduceHpWithRockCake() {
        ...
        if (Health.getCurrent() > 1) {
            if (Inventory.contains(items.dwarvenRock)) {
                DI.get().send(MenuAction.forSpriteItem(Inventory.getItems(items.dwarvenRock).first(), "Guzzle"));
            } else if (Inventory.contains(items.locatorOrb)) {
                DI.get().send(MenuAction.forSpriteItem(Inventory.getItems(items.locatorOrb).first(), "Feel"));
            } else {
    ...
    public static boolean restoreAtFerox(NeckBot<?, ?> bot) {
        ...
        try {
            DI.get().send(MenuAction.forGameObject(pool, "Drink"));
    ...
    public static boolean canNpcContactWithSwap(HouseConfig.Altar altar) {
        var canSwitch = Magic.Book.LUNAR.isCurrent() || altar.supportsSwitchTo(Magic.Book.LUNAR);
        return canSwitch && canNpcContact(Magic.Book.LUNAR);

    }

    public static boolean canNpcContact() {
        return canNpcContact(Magic.Book.getCurrent());
    }

    private static boolean canNpcContact(Magic.Book book) {
        if (!Magic.Book.LUNAR.equals(book)) return false;
    ...
    public static boolean hasHouseTeleport() {
        return hasHouseTeleport(util.inventoryEquipmentSource());
    }

    public static boolean hasHouseTeleport(Collection<SpriteItem> itemsSource) {
        return Items.containsAnyOf(itemsSource, items.constructionCape, items.houseTab)
                || TeleportSpellInfo.HOUSE.canCast(itemsSource);
    }

    public static TraverseMethod.TraverseBank getHouseTeleport() {
        return getHouseTeleport(Magic.Book.getCurrent(), 10);
    }

    public static TraverseMethod.TraverseBank getHouseTeleport(int count) {
        return getHouseTeleport(Magic.Book.getCurrent(), count);
    }

    public static TraverseMethod.TraverseBank getHouseTeleport(Magic.Book book, int count) {
        if (Skill.CONSTRUCTION.getBaseLevel() == 99)
            return new TraverseMethod.TraverseBank(NeckBank.toMap(1, items.constructionCape));
        else if (Objects.equals(book, Magic.Book.STANDARD)) {
            return TraverseMethod.TraverseBank.getHouseBank(book, count);
        }
        return new TraverseMethod.TraverseBank(NeckBank.toMap(count, items.houseTab));
    }
    ...
    public static List<SpriteItem> inventoryEquipmentSource() {
        var inventory = Inventory.getItems();
        var equ = Equipment.getItems();
        var h = new ArrayList<>(inventory);
        h.addAll(equ);
        return h;
    }

    public static List<SpriteItem> inventoryEquipmentBankSource() {
        var inventory = Inventory.getItems();
        var equ = Equipment.getItems();
        var h = new ArrayList<>(inventory);
        h.addAll(equ);
    ...
    public static SpriteItem getItem(List<SpriteItem> items, Pattern item) {
        return items.stream()
                .filter(i -> {
    ...
    public static SpriteItem getItemInventoryEquipment(Pattern item) {
        return getItem(inventoryEquipmentSource(), item);
    }
    ...
    public static boolean isInCombat() {
        var p = Players.getLocal();
        if (p == null) return false;
        var hasHealthBar = p.getHealthGauge() != null;
        if (hasHealthBar) return true;
        var anyTargeting = Npcs.newQuery().filter(n -> Objects.equals(n.getTarget(), p)).actions("Attack").results().size() > 0;
        return anyTargeting;
    }

    public static boolean isEquippable(@Nullable SpriteItem item) {
        if (item == null) return false;
        var def = item.getDefinition();
        if (def == null) return false;
        return def.getInventoryActions().stream().anyMatch(s -> Inventory.EQUIP_ACTIONS.matcher(s).find());
    }

    public static boolean canRestoreAtFerox() {
        return Items.contains(util.inventoryEquipmentSource(), items.ringOfDueling);
    }
}
