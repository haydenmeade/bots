...
public class RestockState extends com.neck_flexed.scripts.common.state.RestockState<DkSettings, DkState> {
    ...
    protected Map<Pattern, Integer>[] getWithdraw() {
        return new Map[]{
                ...
                bot.getHouseConfig().hasBaseJewelleryBox() && bot.getHouseConfig().getPool().equals(HouseConfig.Pool.Ornate)
                        ? Collections.emptyMap()
    ...
    protected Pattern[] getExcept() {
        ...
        return util.concatenate(
                ...
                new Pattern[]{items.runePouch, items.imbuedHeart, items.constructionCape, items.houseTab, items.ringOfDueling,},
