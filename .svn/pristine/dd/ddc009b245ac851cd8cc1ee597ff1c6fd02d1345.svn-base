...
public class bot extends SlayerBotImpl<SlayerSettings> {
    ...
    public @NotNull Location selectLocation(SlayerMonster monster) {
        ...
        var defaultLocation = locations.stream().filter(this::checkCondition).findFirst().orElse(
    ...
    public void setLoadoutOverriders() {
        ...
        var task = Monster.fromSlayerTask(getOverrideTask()).orElse(null);
        this.loadouts.addLoadoutOverrider(new MonsterLoadoutOverrider(task));
        var slaughterTasks = util.parseCsvRegexString(settings().slaughterTasks());
        var expeditiousTasks = util.parseCsvRegexString(settings().expeditiousTasks());

        if (task != null) {
            if (Arrays.stream(task.getAllNames()).anyMatch(s -> Arrays.stream(slaughterTasks).anyMatch(t -> util.checkPatternArrayAgainst(s).test(t)))) {
                this.loadouts.addLoadoutOverrider(new SlayerBraceletOverrider(SlayerBraceletOverrider.Bracelet.Slaughter));
            } else if (Arrays.stream(task.getAllNames()).anyMatch(s -> Arrays.stream(expeditiousTasks).anyMatch(t -> util.checkPatternArrayAgainst(s).test(t)))) {
                this.loadouts.addLoadoutOverrider(new SlayerBraceletOverrider(SlayerBraceletOverrider.Bracelet.Expeditious));
            }
        }
    }
    ...
    public boolean doWeNeedToRestockInFight() {
        ...
        if (mOpt.isPresent()) {
            ...
            if (!l.isEquippedOrInInventory(this.loadouts.getLoadoutOverriders())) {
    ...
    public Magic.Book getDesiredSpellbook(@Nullable SlayerMonster m, HouseConfig houseConfig) {
        ...
        if (!SlayerTask.hasTask() || wantToSkip(m)) {
    ...
    public boolean wantToSkip(SlayerMonster m) {
        if (!settings.skipTasksEnabled()) return false;
        if (m == null) {
            return settings.skipUnhandled();
        }
        return Arrays.stream(m.getAllNames()).anyMatch(name -> SlayerUtil.doesTaskMatch(name, settings().skipTaskList(), false));
    }
    ...
    protected void initFromSettings(SlayerSettings settings) {
        this.loadouts = Loadouts.fromEquipmentLoadouts(this,
                ...
                Triple.of(settings().krakenEquipment(), Loadout.LoadoutRole.Combat, SlayerLoadout.KRAKEN_LOADOUT.toString()),
                Triple.of(settings().waterfiendEquipment(), Loadout.LoadoutRole.Combat, SlayerLoadout.WATERFIEND_LOADOUT.toString())
        );
    ...
    public void onSettingChanged(SettingChangedEvent settingChangedEvent) {
        super.onSettingChanged(settingChangedEvent);
        if (Objects.equals(settingChangedEvent.getKey(), "alch_items")) {
            this.alchListener.setAlchables(util.parseCsvRegexString(settings().alchItems()));
        }
    }
    ...
    public void onStart(String... arguments) {
        ...
        DefaultUI.addPanel(0, this, "Instructions", new Text(
                "This is an intelligent slayer bot\n" +
                        ...
                        "Supports: High Alch --> Be on standard\n\n" +
    ...
    protected LoopState<SlayerState> getState(SlayerState s) {
        ...
        if (monster.isPresent() && (barrageListener == null || !Objects.equals(barrageListener.getMonster(), monster.get())))
            this.barrageListener = new BarrageListener(this, monster.get());
        switch (s) {
            case TRAVERSING:
                if (monster.isEmpty()) {
                ...
                return new TraverseState(this, monster.get(),
                        util.parseCsvRegexString(settings().ignorePlayers()));
            case BARRAGING:
                if (monster.isEmpty()) {
                    return new StartingState(this);
                }
                if (monster.get().getTask().equals(Task.SMOKE_DEVILS)) {
                    return new SmokeDevilBarrageState(this, barrageListener,
                            this.loadouts.getForName(SlayerLoadout.BARRAGE_LOADOUT), settings.spell(), settings.reboostMage(), util.parseCsvRegexString(settings.ignorePlayers()),
                            settings());
                }
                return new BarrageState(this, barrageListener, this.prayerFlicker,
            ...
            case LOOTING:
                return new LootState(this, settings(), monster.orElse(null), settings().buryBones());
            case LURING:
                if (monster.isEmpty()) {
                    return new StartingState(this);
                }
                return new LuringState(this,
                        barrageListener,
                        monster.get(),
                        this.loadouts.getForName(SlayerLoadout.LURE_LOADOUT), util.parseCsvRegexString(settings.ignorePlayers())
                );
            case STACKING:
                if (monster.isEmpty()) {
                    return new StartingState(this);
                ...
                return new StackingState(this, monster.get(), barrageListener);
            case RESTORING:
            ...
            case FIGHTING:
                ...
                return new FightState(this,
                        ...
                        settings(),
                        !settings().antifire().equals(Antifire.None),
                        util.parseCsvRegexString(settings().ignorePlayers())
                );
