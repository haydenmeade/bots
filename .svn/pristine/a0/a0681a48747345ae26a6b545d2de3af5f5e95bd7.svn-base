...
public interface SlayerMonster {
    ...
    default Loadout getLoadout(SlayerBotImpl<?> bot) {
        ...
        } else if (getTask().equals(Task.WATERFIENDS)) {
            l = loadouts.getForName(SlayerLoadout.WATERFIEND_LOADOUT);
        } else if (getTask().equals(Task.CAVE_KRAKEN)) {
            l = loadouts.getForName(SlayerLoadout.KRAKEN_LOADOUT);
        }
