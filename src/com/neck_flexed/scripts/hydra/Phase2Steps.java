...
public class Phase2Steps implements PhaseSteps {
    ...
    public @NotNull PhaseSteps.Step GetNextStep(int idx) {
        switch (idx) {
            case 0:
                return new Step(0, h -> !h.getHydra().getImmunity() || h.getHydra().getNextSpecialRelative() == 1, HydraListener::isNotOverVent, step0);
            case 1:
                return new Step(1, h -> h.playerAttacked() || h.getHydra().getNextSpecialRelative() <= 1, step1);
            case 2:
                return new Step(2, h -> h.playerAttacked() || h.getHydra().getNextSpecialRelative() <= 1, step2);
            case 3:
                return new Step(3, h -> {
