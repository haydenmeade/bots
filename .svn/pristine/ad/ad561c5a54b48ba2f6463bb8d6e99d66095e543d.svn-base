...
public class RestockState extends com.neck_flexed.scripts.common.state.RestockState<HydraSettings, HydraState> {
    ...
    protected Map<Pattern, Integer>[] getWithdraw() {
        return new Map[]{
                ...
                NeckBank.toMap(bot.settings().foodAmount(), bot.settings().food().gameName())
        };
    }
    ...
    protected Pattern[] getExcept() {
        return util.concatenate(
                ...
                new Pattern[]{items.runePouch, items.imbuedHeart, items.constructionCape, items.radasBlessing, items.dramenStaff, items.houseTab, items.bonecrusher},
